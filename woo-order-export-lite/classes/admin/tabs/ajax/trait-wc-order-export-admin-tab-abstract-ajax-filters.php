...
trait WC_Order_Export_Admin_Tab_Abstract_Ajax_Filters {
    ...
    private function print_escaped_json($array) {
	...
	public function ajax_get_products() {
		...
		$this->print_escaped_json( apply_filters( "woe_ajax_get_products", WC_Order_Export_Data_Extractor_UI::get_products_like( $q, $limit ),  $q, $limit ) );
	}
	...
	public function ajax_get_users() {
		$this->check_nonce();
		$q = isset($_REQUEST['q']) ? sanitize_text_field(wp_unslash($_REQUEST['q'])) : '';//phpcs:ignore WordPress.Security.NonceVerification.Recommended
		$this->print_escaped_json( WC_Order_Export_Data_Extractor_UI::get_users_like( $q ) );
	}
	...
	public function ajax_get_coupons() {
		$this->check_nonce();
		$q = isset($_REQUEST['q']) ? sanitize_text_field(wp_unslash($_REQUEST['q'])) : ''; //phpcs:ignore WordPress.Security.NonceVerification.Recommended
		$this->print_escaped_json( WC_Order_Export_Data_Extractor_UI::get_coupons_like( $q ) );
	}
	...
	public function ajax_get_categories() {
		...
		$q = isset($_REQUEST['q']) ? sanitize_text_field(wp_unslash($_REQUEST['q'])) : '';//phpcs:ignore WordPress.Security.NonceVerification.Recommended
		$this->print_escaped_json( WC_Order_Export_Data_Extractor_UI::get_categories_like( $q, $limit ) );
	}
	...
	public function ajax_get_order_custom_fields_values() {
		...
		$this->print_escaped_json( WC_Order_Export_Data_Extractor_UI::get_order_custom_fields_values( $cf_name ) );
	}

	public function ajax_get_user_custom_fields_values() {
        $this->check_nonce();
		$cf_name = isset($_POST['cf_name']) ? sanitize_text_field(wp_unslash($_POST['cf_name'])) : '';//phpcs:ignore WordPress.Security.NonceVerification.Missing
		$this->print_escaped_json( WC_Order_Export_Data_Extractor_UI::get_user_custom_fields_values( $cf_name ) );
	}

	public function ajax_get_product_custom_fields_values() {
        $this->check_nonce();
		$cf_name = isset($_POST['cf_name']) ? sanitize_text_field(wp_unslash($_POST['cf_name'])) : '';//phpcs:ignore WordPress.Security.NonceVerification.Missing
		$this->print_escaped_json( WC_Order_Export_Data_Extractor_UI::get_product_custom_fields_values( $cf_name ) );
	}
	...
	public function ajax_get_products_taxonomies_values() {
		...
		$this->print_escaped_json( WC_Order_Export_Data_Extractor_UI::get_products_taxonomies_values( $tax ) );
	}
	...
	public function ajax_get_products_attributes_values() {
		...
		$this->print_escaped_json( WC_Order_Export_Data_Extractor_UI::get_products_attributes_values( $attr ) );
	}
	...
	public function ajax_get_products_itemmeta_values() {
		...
		$this->print_escaped_json( WC_Order_Export_Data_Extractor_UI::get_products_itemmeta_values( $item ) );
	}

	public function ajax_get_order_shipping_values() {
        $this->check_nonce();
		$item = isset($_POST['item']) ? sanitize_text_field(wp_unslash($_POST['item'])) : '';//phpcs:ignore WordPress.Security.NonceVerification.Missing
		$this->print_escaped_json( WC_Order_Export_Data_Extractor_UI::get_order_meta_values( '_shipping_', $item ) );
	}

	public function ajax_get_order_billing_values() {
        $this->check_nonce();
		$item = isset($_POST['item']) ? sanitize_text_field(wp_unslash($_POST['item'])) : '';//phpcs:ignore WordPress.Security.NonceVerification.Missing
		$this->print_escaped_json( WC_Order_Export_Data_Extractor_UI::get_order_meta_values( '_billing_', $item ) );
	}
	...
	public function ajax_get_order_item_names() {
		...
		$this->print_escaped_json( WC_Order_Export_Data_Extractor_UI::get_order_item_names( $item_type ) );
	}
	...
	public function ajax_get_order_item_meta_key_values() {
		...
		$this->print_escaped_json( WC_Order_Export_Data_Extractor_UI::get_order_item_meta_key_values( $meta_key ) );
	}
